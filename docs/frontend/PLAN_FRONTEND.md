...
| 17 | API REST — Caracterización (6 sectores read-only) | B | **Hecho** | 2026-05-27 — 13 endpoints en /caracterizacion/api/. Insights agregados + (list+detail) por cada sector (Cultura, Deporte, Mujer, Salud, Poblacional, Participación). MujerDetail incluye hogar anidado. SaludDetail expone tiene_firma derivado (firma_mongo_id NUNCA en response — guardia de seguridad con test dedicado). Resolución N+1 de nombres Evento/Persona via dict precomputado en context del serializer. 8 tests nuevos. |
