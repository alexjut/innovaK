...
| N18 (media) | Pestañas N18 ahora muestran panel KPIs inline al click: badges "X eventos · Y próximos · Z ejecutados" calculados al render (Evento.objects con Q(fecha_inicio__gte=hoy)). Persistencia LocalStorage (`kennedy.subgrupoTab`): la última pestaña activa se restaura al recargar la página. Cubre los 17 subgrupos dep_id=3. Plan alta (URLs propias por subgrupo) sigue abierto (`e1fdee6`) |
| N27 | Limpieza de datos sucios en BD con scripts SQL puntuales. (1) Subgrupo id=3 `Prticipación` → `Participación` (typo). (2) Subgrupos `Seguridad` duplicados: merge id=5 → id=38 (migra 1 funcionario + 4 eventos, DELETE id=5). (3) Usuario id=9 `Coordionador` → `Coordinador`; eliminados 5 grupos sobrantes (Admin/UsuarioGeneral/Docente/Lider/LiderParticipacion), queda solo `Coordinador`. Vínculo Persona/Funcionario a Cultura queda pendiente (requiere nombre real del humano). Backup: `poblacion_kennedy_pre_n27_20260511_105806.dump` (`9e56515`) |
| C5 | Decisión: Alex revocó la excepción "votaciones en inglés" de CLAUDE.md §3. Rename completo a español ejecutado por agente backend en `apps/votaciones/`: 7 archivos renombrados (`event.py`→`evento.py`, etc.), 20 archivos modificados, 4 clases (Event→Evento, Voter→Votante, Vote→Voto, Candidate→Candidato), service `register_vote()`→`registrar_voto()` y `VoteResult`→`ResultadoVoto`. **Preservados sin tocar**: `db_table` (`votaciones_event` etc. siguen igual), columnas BD en inglés, URL names en `urls.py`, parámetros URL `event_id`/`candidate_id`, claves JSON del API público (contrato frontend). HTTP 200 en 11 endpoints probados. Smoke 128/128 OK. CLAUDE.md §3 debe actualizarse para eliminar la mención "Excepción: votaciones". (`f517899`) |
| permisos | Limpieza de privilegios: usuario `Lider-Inv` perdió grupo `Admin` (quedó solo `LiderParticipacion`, 6 módulos vs 19 antes). Usuario `ParticipacionAdmin` cambió de grupo `Admin` a `LiderParticipacion` (coherente con su nombre). Cache global de permisos invalidado. Cero exceso de privilegio en usuarios no-superuser (`f517899`). |
| Jazzmin | Tema moderno activado para Django admin (`django-jazzmin==2.6.0` ya estaba en requirements pero nunca se cargó). INSTALLED_APPS incluye `jazzmin` antes de `django.contrib.admin`. `JAZZMIN_SETTINGS` con branding institucional Alcaldía Kennedy: navbar/sidebar rojo (`navbar-danger`, `sidebar-dark-danger`), íconos FontAwesome por app, link "Volver al sistema" en topmenu, modo dark disponible (`darkly`). Admin responde 302 (redirect login esperado) (`f517899`). |
| N22 | UNIQUE INDEX parcial creado: `idx_beneficiario_persona ON beneficiario(persona_id) WHERE tipo='PERSONA' AND persona_id IS NOT NULL`. Auditoría pre-cambio: 0 duplicados existentes (3580 Beneficiarios PERSONA limpios). Código defensivo en `beneficiario_helpers.py`: `except IntegrityError → refetch` retorna el Beneficiario que creó la request paralela en lugar de devolver None. Backup: `poblacion_kennedy_pre_n22_20260511_161018.dump` (`68e2083`) |
| C4 | **Falsa alarma**. La auditoría reveló que las 10 columnas `upz_codigo`/`barrio_codigo` en BD (barrio, escuela×2, lugar×2, parque, emprendimiento×2, inscripcion_banco_iniciativa, localidad) **ya tienen FK formales** desde antes (`barrio_upz_codigo_fkey`, `escuela_barrio_codigo_fkey`, etc.). Cero huérfanos en toda la BD. El item descriptivo en deuda era inexacto. Los modelos Django siguen como `IntegerField` por compatibilidad con call sites (`b.upz_codigo` directo en views/templates); convertir a `ForeignKey` rompería 8+ call sites sin beneficio funcional adicional. **Cierre documental sin código.** |
| N19 | Form Banco rediseñado: `rep_nombre` (1 campo libre) → 4 campos separados (`rep_nombre1`/`rep_nombre2`/`rep_apellido1`/`rep_apellido2`). JS autollenado al perder foco de `rep_numero_doc` consulta `/caracterizacion/api/persona/?doc=X` (con rate limit N23). Si la persona existe, autollena los 4 campos; si no, queda vacío para que el usuario los rellene. En `save()`, `obtener_o_crear_persona()` crea/recupera Persona; `asegurar_beneficiario_persona()` garantiza Beneficiario PERSONA siempre. El campo legacy `rep_nombre` en BD se sigue guardando como nombre completo derivado de los 4 campos (compat reportes). (`d003c6d`) |
| N21 | `SECTORES_META` ahora incluye `subgrupo_id` explícito (Cultura→1, Deporte→2, Mujer→40, Salud→45, Poblacional→None, Participación→3). Helpers nuevos: `subgrupo_id_de_sector(codigo)` y `sector_de_subgrupo_id(sid)`. Los 6 wizard views ahora leen `sector_label` desde `SECTORES_LABEL[SECTOR_X]` en lugar de hardcoded ("Cultura", "Deporte", etc.) — única fuente de verdad. Si Alex renombra un sector en SECTORES_META, todos los renderings y reportes se actualizan automáticamente sin tocar más código (`4bb3fe4`). |

---

## 2026-05-25 — Cierre convenciones cosméticas (C2, C3, C6)

Sesión corta de alineación quirúrgica modelo ↔ BD usando agente `bd`
para mapear mismatches reales (no por convención, sino por divergencia
real con `poblacion_kennedy`). Total 0 ítems abiertos.

| ID | Resumen |
|----|---------|
| C2 | **0 mismatches reales.** Auditoría con agente bd confirmó que las 109 FKs sin `db_column` explícito siguen el patrón implícito de Django (`<field>_id`) que coincide con la columna real en BD. Las FKs con nombre Python distinto al de la columna (p.ej. `lugar_nacimiento` → `lugar_nacimiento_codigo`) **sí** declaran `db_column` explícito. **Cierre documental** — la convención ya se respeta. |
| C3 | **~25 PKs alineados al tipo real de BD.** Patrón dominante: `BigAutoField` declarado donde BD es `integer` → cambiado a `AutoField`. Afecta `Persona`, `Participante`, `PersonaDocumento`, `EstadoCivil`, `Lugar`, `GeoReferenciacion`, `LugarIncidencia`, 9 modelos en `kactivo/kasistencia.py` (Docente, Curso, Disciplina, Grupo, Clase, HorarioClase, Convocatoria, TipoAsistencia, ClaseParticipante), 4 en `kactivo/kdocumentos.py`, `Crp`. Inverso en `votaciones/`: 4 modelos (`Evento`, `Candidato`, `Votante`, `Voto`) sin PK declarada → agregada `id = BigAutoField(primary_key=True)` (BD es `bigint`). Además se borraron 5 modelos muertos en `models_auxiliares.py` (`TipoRedSocial`, `NivelSocioeconomico`, `TenenciaVivienda`, `TipoSalud`, `TipoSangre`) que apuntaban a tablas inexistentes en BD y nunca eran importados desde ningún sitio. |
| C6 | **~45 FKs alineadas con FK real de BD.** Como BD es `managed=False`, los `on_delete=CASCADE/SET_NULL/PROTECT` que contradicen la política real de la columna (`NO_ACTION` en BD) son código que miente al lector. Estrategia: cambiar a `DO_NOTHING` para reflejar realidad. **Conservado**: `PROTECT` donde aplica (PROTECT≈NO_ACTION en efecto neto — ambos bloquean — pero PROTECT da mejor UX en errores). Afectados: `Persona` (24 FKs), `Funcionario` (5 FKs), `Subgrupo.dependencia`, `Participante.persona`, `Inscripcion` (3 FKs), `Proveedor` (2 FKs), `LugarNacimiento` (4 FKs), `Lugar` (3 FKs UPZ/barrio/localidad), `HorarioClase.clase`, `DocumentoEvento` (2 FKs), `DocumentoParticipante.tipo_archivo`, `Proyecto.programa`, `ProyectoInversionItem.cdp`, `Voto` (2 FKs candidato), 5 FKs SET_NULL del Banco. |

**Tests:** 128/128 OK al cierre (sin regresiones — los cambios son cosméticos puros, no afectan runtime con `managed=False`).
