...
| N13 | ALTA | Tablas puente M2M de Banco de Iniciativas (`inscripcion_banco_escenario`, `inscripcion_banco_implemento`, `inscripcion_banco_rango_etario`, `inscripcion_banco_enfoque`, `inscripcion_banco_beneficio_alk`) tienen PK compuesta `(inscripcion_id, codigo)` en BD pero Django asume PK simple `id`. Al hacer `insc.escenarios.set(...)` falla con `column ... id does not exist`. **Workaround**: por ahora los multiselects M2M no persisten desde el form. **Solución**: o agregar `id BIGSERIAL UNIQUE NOT NULL` a las 5 tablas vía DDL, o declarar `primary_key=True` en una FK del modelo Django. |
