...
| C3 | **Parcial ✅ 2026-08-05** · resto pendiente | **Hecho**: (a) columna `synced_at` **unificada** en las 3 tablas que usaban otro nombre (`ingerido_en`/`sincronizado_at`) — DDL de metadatos aplicado + 2 modelos y 3 comandos actualizados; (b) **licencias** declaradas como constante (`core/licencias.py`, 7 fuentes CC BY 4.0) e impresas por el orquestador. (c) **seco-por-defecto** en los 5 comandos que escribían sin flag (`--dry-run`→`--write`, invertido) + orquestador y tests al día: ahora una corrida manual NO escribe salvo `--write`. (d) **helper `SyncOficialCommand`** (`core/sync_oficial.py`) con `--write`, `hash_fila()` y `upsert()` de una sola sentencia (`INSERT … ON CONFLICT … DO UPDATE`, inyecta las columnas espejo), **probado en aislamiento** (10 tests, sin tocar la BD; uno verifica que reproduce el SQL que `sync_placas` ya usa). **✅ COMPLETADO 2026-08-06**: DDL 021 aplicado (13 columnas espejo, conteos intactos) y **los 5 comandos cableados**, cada uno verificado corriéndolo. La regla de «uno por uno, no a ciegas» se ganó su sueldo: **ninguno de los cinco upserts tenía el mismo guardia**, y tres de ellos se habrían roto en silencio. `sync_cai` protege FILAS con `WHERE fuente='SCJ'` (probado en vivo marcando un CAI como MANUAL: sobrevivió intacto); `sync_colegios` protege una COLUMNA —`estrato_ideca`— por omisión, verificado con una huella md5 que quedó idéntica; `sync_estratificacion` protege un VALOR POR FILA, la vigencia por manzana, que el atajo del helper habría aplanado borrando dos excepciones de 2018 entre 18.929 filas. 🔴 **Hallazgo al correr `sync_placas`: la capa está a medio cargar** — 1,78 M de ~3,54 M. El geocodificador de direcciones se apoya en ella y hoy resuelve sobre la mitad del universo sin que nada lo advierta. Completarla son ~10 min medidos, pero es carga de datos y va aparte. `fecha_corte` (cai, colegio) NO se renombra: la leen muchas vistas y es la convención de fecha-de-fuente de esas tablas. **Scope de estratificación**: **Bogotá** (decidido por Alex 2026-08-05 — el geocoder del borde lo necesita; el mapa ya recorta a Kennedy al servir) |
