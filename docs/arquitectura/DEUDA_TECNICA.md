...
| **F10** | ALTA | 🔴 **Un `$` dentro de un valor del `.env` se lo come Docker Compose, y la divergencia es silenciosa.** Compose **interpola** los valores del `.env` al pasarlos por su lista `environment:`; `SECRET_KEY` contenía `$np` y Compose lo sustituyó por nada — 50 caracteres en el archivo, **47 en el proceso**. No falla nada: solo divergen. El riesgo es el despliegue siguiente, o el día que alguien «arregle» el `$`: cambia la clave efectiva y **caen todas las sesiones y todos los enlaces de restablecimiento**. Alineado el 2026-08-06 escribiendo en el `.env` la clave de 47 que ya corría (la dirección contraria a la intuitiva: escapar el `$` habría roto las sesiones en el acto). **Regla:** si un valor lleva `$`, no pasarlo por el compose, o escaparlo `$$`. Queda por auditar si otros valores del `.env` tienen `$` | `docker-compose.yml` · `core/settings.py:43` |
| **F11** | MEDIA | **`docker compose restart` NO relee el `.env` ni el compose** — reusa el contenedor con su entorno viejo; hace falta `up -d`. Varios comentarios y documentos dicen «reiniciar», que es exactamente lo que no basta. Costó un diagnóstico el 2026-08-06 | — |
| **F12** | BAJA | **Dos caminos para las variables de entorno, y no está dicho en ningún lado.** `settings.py` llama `load_dotenv()`, así que el `.env` montado alcanza para que una variable llegue a `os.environ` **aunque el compose no la nombre**; y `load_dotenv` **no pisa** lo que ya está en el entorno, así que cuando viaja por los dos caminos gana la del compose. Documentado en `settings.py` el 2026-08-06 tras diagnosticarlo mal primero: medir con `docker exec … sh -c 'echo $VAR'` mira el entorno del **shell**, no lo que ve Django | `core/settings.py:1-4` |
