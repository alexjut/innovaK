...
CREATE TABLE IF NOT EXISTS formulacion_transicion (
    -- `id` propio: misma lección N3 que en formulacion_contrato. «Aprobada»
    -- tiene tres destinos y sobre una PK compuesta Django perdería dos.
...
CREATE TABLE IF NOT EXISTS formulacion_contrato (
    -- `id` propio ADEMÁS de la PK compuesta, y no es adorno: es la lección N3
    -- del 2026-05-11 de este mismo repo. Django necesita una PK de UNA columna;
    -- sobre una PK compuesta mapea la primera como si fuera la llave y PIERDE
    -- filas en silencio cuando hay más de una. Le pasó a `contrato_proyecto` y
    -- a `contrato_actividad`, y acá el caso es seguro: el contrato 98 toca
    -- siete actividades.
    id             bigint GENERATED BY DEFAULT AS IDENTITY UNIQUE,
    formulacion_id bigint NOT NULL REFERENCES formulacion(id) ON DELETE CASCADE,
