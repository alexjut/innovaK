...
#: `2024404`, `20241158`. Año pegado al consecutivo, sin separador. 647 filas.
#:
...
#: `934 2025`, `939 2025`: el mismo formato del guion con un espacio. Son 2
#: filas y podrían ignorarse, pero dejarlas fuera significaría no enganchar
#: dos contratos reales por un carácter.
...
def normalizar_numero_compromiso(raw) -> tuple[int | None, int | None]:
    ...
    FDLK` —honorarios de ediles, que no son contratos— y otras 24 en diez
    formatos más: `EPS017`, `RES 541`, `149957-2025`. Forzar un número para
    ellas colgaría el CRP del contrato equivocado, que es peor que dejarlo
    suelto: un vacío se ve y se corrige, un enganche falso se propaga.

    Los formatos que sí se aceptan salieron de medir el archivo completo, no
    de suponer:

        1.914  NNN-AAAA        351-2026, 002-2026
          647  AAAAnnn         2024404  (año pegado al consecutivo)
           45  EDIL n FDLK     → (None, None), a propósito
            2  NNN AAAA        934 2025 (espacio en vez de guion)
            1  CPS-NNN-AAAA    CPS-858-2025
           21  otros           EPS017, RES 541, 149957-2025 → (None, None)
    """
    if raw is None:
        return (None, None)
...
#: termina en el mismo código— sobre las 1.028 filas de inversión del
...
def proyecto_de_rubro(rubro) -> str | None:
    ...
    (`O230689`, `O230690` — 1.602 filas, el 61 % del archivo) y los de
    funcionamiento (`O21…`) no identifican proyecto: devuelven `None` y su
    fila se resuelve después cruzando el contrato contra el CRP de la
    vigencia original.
    """
    if not rubro:
        return None
...
def tipo_de_rubro(rubro) -> str:
    ...
    Decide si la fila puede enganchar proyecto. Medido en el archivo: 1.028
    de inversión, 1.602 de obligaciones por pagar (61 %) y 152 de
    funcionamiento.
    """
    if not rubro:
        return "desconocido"
