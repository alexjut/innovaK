...
class DocumentoParticipante(models.Model):
    ...
    tipo_archivo = models.ForeignKey(TipoArchivo, on_delete=models.DO_NOTHING, null=True, db_column='tipo_archivo_id')
...
class DocumentoEvento(models.Model):
    id = models.AutoField(primary_key=True)
    evento = models.ForeignKey(Evento, on_delete=models.DO_NOTHING, db_column='evento_id')
    tipo_archivo = models.ForeignKey(TipoArchivo, on_delete=models.DO_NOTHING, null=True, db_column='tipo_archivo_id')
    nombre_archivo = models.CharField(max_length=255)
