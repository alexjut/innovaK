...
function getBadgeClass(t) {
  return t === "deporte" ? "bg-success"
       : t === "cultura" ? "bg-primary"
       : t === "educacion" ? "bg-warning"
       : "bg-secondary";
}
...
function buildQuery() {
  ...
  const pick = sel => sel ? [...sel.selectedOptions].map(o => o.value).filter(Boolean) : [];
  ...
  subg.forEach(v => p.append("subgrupo", v)); // 'sub1'/'sub2' también sirven (backend los convierte a int)
  if (q) p.set("q", q);
  return p.toString();
}
...
function initKennedy() {
    ...
    features.forEach(f => {
      ...
      const html = `
        <div class="marker-popup">
          ...
          <div><strong>UPZ:</strong> ${p.upz_nombre || ""}</div>
          <div><strong>Barrio:</strong> ${p.barrio_nombre || ""}</div>
          <div><strong>Tipo:</strong> <span class="badge ${getBadgeClass(p.tipo)}">${(p.tipo || "").toUpperCase()}</span></div>
  ...
  async function cargar() {
    try {
      ...
      renderMapFromFeatures(Array.isArray(geo?.features) ? geo.features : []);
      updateKPIsFromAPI();
    } catch (e) {
  ...
  // ===== Capas de polígonos =====
  function styleUPZ()       { return { color: "#198754", weight: 1, fillColor: "#198754", fillOpacity: 0.08 }; }
  function styleBarrios()   { return { color: "#0d6efd", weight: 1, fillColor: "#0d6efd", fillOpacity: 0.08 }; }
  function styleLocalidad() { return { color: "#dc3545", weight: 2, dashArray: "4 2", fillOpacity: 0 }; }
  ...
  function withInteractions(kind, nameField = "nombre") {
    return {
      ...
      onEachFeature: (feature, l) => {
        ...
        l.on("mouseover", e => { e.target.setStyle({ weight: 3, fillOpacity: 0.18 }); e.target.bringToFront(); });
        l.on("mouseout",  e => {
  ...
  async function toggleGeoLayer(url, layerObj, { bringToBack = false, fit = false } = {}) {
    try {
      ...
      if (bringToBack) layerObj.bringToBack(); else layerObj.bringToFront();
      map.addLayer(layerObj);
      if (fit && layerObj.getLayers().length) {
        try { map.fitBounds(layerObj.getBounds(), { padding: [20, 20] }); } catch {}
      }
      return layerObj;
    } catch (e) {
  ...
  cbBarrios?.addEventListener("change", async function () {
    if (this.checked) {
      const upzSel = [...(document.getElementById("f-upz")?.selectedOptions || [])].map(o => o.value).filter(Boolean);
  ...
  // SIN código hardcodeado: usamos el endpoint /api/localidad/kennedy
  ...
  async function refreshBarriosPolygons({ fit = false } = {}) {
    ...
    try {
      ...
      if (fit && barriosLayer.getLayers().length) {
        try { map.fitBounds(barriosLayer.getBounds(), { padding: [20, 20] }); } catch {}
      }
  ...
  window.__kennedy = { map, layers: { upzLayer, barriosLayer, localidadLayer }, cargar };
}

// Fuerza navegación en botones/links que a veces quedan bloqueados por handlers globales
function forceAnchorNavById(id) {
  const el = document.getElementById(id);
  if (!el) return;
  const clone = el.cloneNode(true);           // quitamos listeners previos
